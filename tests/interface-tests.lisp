...
(test command-dispatch-mechanism
  ...
  (let ((invocation-var (find-symbol "*INVOCATION*" :skyline-tool)))
    ...
    (is (listp (symbol-value invocation-var))
        "*invocation* should be a property list")))
...
(test write-master-makefile-generates-valid-makefile
  ...
  (let ((original-makefile-size (when (probe-file "Source/Generated/Makefile")
    ...
    (when (probe-file "Source/Generated/Makefile")
      (let ((new-size (with-open-file (stream "Source/Generated/Makefile")
                        (file-length stream))))
  ...
  (let ((root (skyline-tool::project-root)))
    ...
    (let ((out (merge-pathnames "Source/Generated/Intv/AssetIncludes.s" root))
      ...
      (let ((content (uiop:read-file-string out))
        ...
        (is (search "AssetCatalog:" cat)
            "AssetCatalog.s should contain lookup table for unique asset IDs")
        (is (search "AssetCatalogStride    EQU     6" cat)
            "AssetCatalog stride should be six words after id (gram, map, cols, rows, ngram, bank)")
        (is (search "Blob_TitleCard_GRAM_DATA" cat)
            "AssetCatalog should map TitleCard ID to GRAM_DATA label")
        (is (search "Blob_TitleCard_TILE_MAP" cat)
            "AssetCatalog should map TitleCard ID to TILE_MAP label")))))
    ...
    (when (and (probe-file forth-out) (probe-file mame-out))
      (with-open-file (forth-stream forth-out)
        (with-open-file (mame-stream mame-out)
          (let ((forth-content (make-string (file-length forth-stream)))
                (mame-content (make-string (file-length mame-stream))))
    ...
    (when (and (probe-file output1) (probe-file output2))
      (with-open-file (stream1 output1)
        (with-open-file (stream2 output2)
          (let ((content1 (make-string (file-length stream1)))
                (content2 (make-string (file-length stream2))))
