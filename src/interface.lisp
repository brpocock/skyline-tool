...
(defun run-for-port (port-label &rest subcommand)
  ...
  (handler-case
      (let* ((json-path (make-pathname :directory '(:relative :up)
                                       :name (format nil "Project.~a" port-label) :type "json")))
        (format t "~&DEBUG: json-path = ~a~%" json-path)
        (let ((project-data (json:decode-json-from-source json-path)))
          (format t "~&DEBUG: loaded JSON, project-data keys: ~a~%" (mapcar #'car project-data))
          (setf *project.json* project-data
                *game-title* (cdr (assoc "Game" project-data :test #'string=))
                *part-number*  (cdr (assoc "PartNumber" project-data :test #'string=))
                *studio* (cdr (assoc "Studio" project-data :test #'string=))
                *publisher* (cdr (assoc "Publisher" project-data :test #'string=))
                *machine* (cdr (assoc "Machine" project-data :test #'string=))
                *sound* (cdr (assoc "Sound" project-data :test #'string=))
                *common-palette* (mapcar #'intern (cdr (assoc "CommonPalette" project-data :test #'string=)))
                *default-skin-color* (cdr (assoc "DefaultSkinColor" project-data :test #'string=))
                *default-hair-color* (cdr (assoc "DefaultHairColor" project-data :test #'string=))
                *default-clothes-color* (cdr (assoc "DefaultClothesColor" project-data :test #'string=)))
          (format t "~&DEBUG: run-for-port set *machine* to ~a (type: ~a)~%" *machine* (type-of *machine*))))
    (error (e)
      (format t "~&DEBUG: JSON parsing failed: ~a~%" e)
      (error e))))
