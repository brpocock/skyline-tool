...
(defun atarivox-basic-pronunciation (word)
  "Generate a fallback pronunciation for WORD using SpeakJet phonemes.
   ...
   Emits only valid phonemes from +speakjet-phonemes+."
  (let ((s (string-upcase word))
        (phonemes nil)
        (i 0)
        (len (length word)))
      ...
      (loop while (< i len)
            do (let ((c (peek 0)))
                 (cond
                    ...
                    ((match "EU") (emit "UW"))
                    ((match "ER") (emit "AXRR"))
                    ((match "AR") (emit "AY") (emit "RR"))
                    ((match "OR") (emit "OH") (emit "RR"))
                    ((match "IR") (emit "AXRR"))
                    ((match "UR") (emit "AXRR"))
                    ((char= c #\A)
                     (cond ((char= (peek 1) #\E) (emit "AY") (incf i 2))
                           ((char= (peek 1) #\R) (emit "AY") (emit "RR") (incf i 2))
                    ...
                    ((char= c #\C)
                     (emit (if (member (peek 1) '(#\E #\I #\Y)) "SE" "KO")) (incf i))
                    ((char= c #\D) (emit "DE") (incf i))
                    ((char= c #\E)
                     (cond ((and (char= (peek 1) #\A) (char= (peek 2) #\R))
                            (emit "EY") (emit "AX") (incf i 3))
                    ...
                    ((char= c #\F) (emit "FF") (incf i))
                    ((char= c #\G)
                     (emit (if (member (peek 1) '(#\E #\I #\Y)) "JH" "KO")) (incf i))
                    ((char= c #\H) (emit "HE") (incf i))
                    ((char= c #\I)
                     (cond ((char= (peek 1) #\R) (emit "AXRR") (incf i 2))
                           ((and (char= (peek 2) #\E) (not (alpha-char-p (peek 3))))
                            (emit "IY") (incf i 3))
                           (t (emit "IH") (incf i))))
                    ((char= c #\J) (emit "JH") (incf i))
                    ((char= c #\K) (emit "KO") (incf i))
                    ...
                    ((char= c #\O)
                     (cond ((and (char= (peek 1) #\O) (char= (peek 2) #\R))
                            (emit "OH") (incf i 3))
                           ((char= (peek 1) #\R) (emit "OH") (incf i 2))
                           ((and (char= (peek 2) #\E) (not (alpha-char-p (peek 3))))
                            (emit "OW") (incf i 3))
                           (t (emit "AY") (incf i))))
                    ((char= c #\P) (emit "PE") (incf i))
                    ((char= c #\Q) (emit "KO") (incf i))
                    ...
                    ((char= c #\V) (emit "VV") (incf i))
                    ((char= c #\W) (emit "WW") (incf i))
                    ((char= c #\X) (emit "KO") (emit "SE") (incf i))
                    ((char= c #\Y) (emit "IY") (incf i))
                    ((char= c #\Z) (emit "ZZ") (incf i))
                    (t (incf i))))))
     (nreverse phonemes)))
