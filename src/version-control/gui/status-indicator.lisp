...
(clim:define-presentation-type vc-status-indicator () )

;;
;; Status Indicator Command Table
;;

(clim:define-command-table vc-status-menu)

(clim:define-command (com-vc-stage :command-table vc-status-menu :menu t :name t)
    ((file 'pathname :gesture :select))
  (let ((status (skyline-tool.version-control:vc-file-status file)))
    (case status
      (:staged nil) ; Already staged
      ((:modified :untracked) 
       (skyline-tool.version-control:vc-add (skyline-tool.version-control:make-git-backend) (list file))
       (skyline-tool.version-control:vc-commit (skyline-tool.version-control:make-git-backend) "Staged via indicator"))
      (t (format t "Cannot stage ~a in status ~a~%" file status)))))

(clim:define-command (com-vc-revert :command-table vc-status-menu :menu t :name t)
    ((file 'pathname :gesture :select))
  (skyline-tool.version-control:vc-reset (skyline-tool.version-control:make-git-backend) (list file)))

(clim:define-command (com-vc-compare :command-table vc-status-menu :menu t :name t)
    ((file 'pathname :gesture :select))
  (show-vc-diff-dialog file))

;;
;; Status Indicator Presentation
;;

(defun present-vc-status (stream file-path &key (style ':compact))
  "Present VC status indicator for FILE-PATH.
   STYLE can be :compact (small icon bottom-right) or :full (bottom bar)."
  (let ((status (skyline-tool.version-control:vc-file-status file-path)))
    (clim:with-output-as-presentation
        (stream (cons :vc-status file-path) 'vc-status-indicator)
      (case style
        (:compact (present-vc-status-icon stream status))
        (:full (present-vc-status-bar stream status))))))

(defun present-vc-status-icon (stream status)
  "Present compact status icon"
  (let ((icon (skyline-tool.version-control:vc-status-icon status)))
...
(defun present-vc-status-bar (stream status)
  "Present full status bar at bottom of pane"
  (let ((text (skyline-tool.version-control:vc-status-text status))
        (color (case status
