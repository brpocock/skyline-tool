  ...
  (let* ((mr (and (typep stream 'clim:sheet)
         ...
         (win-h (if mr
                    (multiple-value-bind (x1 y1 x2 y2)
                      ...
                      (- y2 y1))
                    600))
         (min-scale (max (/ 8 (max 1 width)) (/ 4 (max 1 height))))
         (max-scale (min (* 0.8 (/ win-w (max 1 width)))
...
(defun %print-clim-pixels (palette-pixels stream &key (max-width 300) (max-height 200))
  (let* ((width (array-dimension palette-pixels 0))
         ...
         (cap-scale (cond ((and win-w win-h)
                           (min (* 0.8 (/ win-w (max 1 width)))
                                (* 0.8 (/ win-h (max 1 height)))))
                          (t (min (/ max-width (max 1 width))
...
(defun %compute-ansi-sizing (width height)
  (let* ((cols-str (uiop:getenv "COLUMNS"))
         ...
         (target-rows (max 4 (round (* target-cols (/ height width)))))
