...
(defun %print-thumbnail-clim (stream png-file width height)
  (let* ((pixel-aspect (if (> width 160) 1 2))
         (vis-width (* width pixel-aspect))
         (mr (and (typep stream 'clim:sheet)
         ...
         (win-h (if mr
                    (multiple-value-bind (x1 y1 x2 y2)
                      ...
                      (- y2 y1))
                    600))
         (min-scale (max (/ 8 (max 1 vis-width)) (/ 4 (max 1 height))))
         (max-scale (min (* 0.8 (/ win-w (max 1 vis-width)))
                         (* 0.8 (/ win-h (max 1 height)))))
         (scale (max min-scale (min max-scale 1.0)))
         (out-w (max 1 (floor (* vis-width scale))))
...
(defun %print-clim-pixels (palette-pixels stream &key (max-width 300) (max-height 200))
  (let* ((width (array-dimension palette-pixels 0))
         ...
         (win-h (if mr
                    ...
                    nil))
         (min-scale (max (/ 8 (max 1 vis-width)) (/ 4 (max 1 height))))
         (cap-scale (cond ((and win-w win-h)
                           (min (* 0.8 (/ win-w (max 1 vis-width)))
                                (* 0.8 (/ win-h (max 1 height)))))
                          (t (min (/ max-width (max 1 vis-width))
                                  (/ max-height (max 1 height))))))
         (scale (max min-scale (min cap-scale 1.0)))
         (x-scale (* scale pixel-aspect))
         (out-w (max 1 (floor (* vis-width scale))))
         (out-h (max 1 (floor (* height scale)))))
    (clim:with-room-for-graphics (stream :height out-h)
      (dotimes (y height)
            ...
            (when pixel
              (let ((rgb (elt (machine-palette) pixel)))
                ...
                (clim:draw-rectangle* stream
                                      (floor (* x x-scale)) (floor (* y scale))
                                      (ceiling (* (1+ x) x-scale)) (ceiling (* (1+ y) scale))
                                      :filled t))))))
      (setf (clim:medium-ink stream) clim:+foreground-ink+))))
