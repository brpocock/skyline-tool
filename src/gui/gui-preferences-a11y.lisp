(defun make-a11y-tab-pane (frame)
  (flet ((update (key value)
    ...
    (let* ((color-theme (get-pref '(:accessibility :color-theme) :normal))
           ...
           ;; Color theme radio group (all in one vertical stack)
           (color-buttons
      ...
      (clim:vertically (:name 'a11y-tab-pane :spacing 12)
        ;; Color Theme section (vertical stack)
        (clim:horizontally (:spacing 4)
          (clim:labelling (:label "Color Theme:" :text-style (clim:make-text-style nil :bold :normal)))
          color-box)
        ;; Sunrise/Sunset row (horizontal arrangement)
        sunrise-row
        ;; Vertical spacer between color theme and shortcuts
        (clim:labelling (:label " "))
        ;; Keyboard Shortcuts section (vertical stack)  
        (clim:horizontally ()
