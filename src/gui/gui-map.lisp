    ...
    (clim:formatting-row (stream)
      ...
      (clim:formatting-cell (stream :align-x :left)
        (interactive-editing-gadget-with-validation
         stream resource
         :setter (lambda (r) (game-resource-title r))
         :getter (lambda (r v) (rename-asset r v))
         :label "Name:"
          ...
          (dotimes (x (min grid-w map-w))
            (let* ((gid (aref gids x y)))
              (when (plusp gid)
                ...
                (let ((ts-info (find gid (game-resource-tilesets data)
...
(defun display-map-inspector (frame pane)
  (let* ((data (map-inspector-frame-data frame))
    ...
    (let* ((display-name (game-resource-display-name data))
           (suffix (or (map-inspector-new-suffix frame)
                       (game-resource-numeric-suffix data))))
      ;; Header: map name
      ...
      (format stream "~d × ~d tiles" (game-resource-width data) (game-resource-height data))
      (terpri stream) (terpri stream)
      ;; Tilesets
      (clim:with-text-style (stream (clim:make-text-style :sans-serif :bold :normal))
        (format stream "~&Tilesets:"))
      (dolist (ts (game-resource-tilesets data))
      ...
      (let ((props (game-resource-properties data)))
        (when props
          (terpri stream)
          (clim:with-text-style (stream (clim:make-text-style :sans-serif :bold :normal))
      ...
      (terpri stream)
      ;; Spawns
      (let ((spawns (game-resource-spawns data)))
        (when spawns
          (clim:with-text-style (stream (clim:make-text-style :sans-serif :bold :normal))
      ...
      (terpri stream)
      ;; Run Commands
      (let ((rcs (game-resource-run-commands data)))
        (when rcs
          (clim:with-text-style (stream (clim:make-text-style :sans-serif :bold :normal))
          ...
          (dolist (rc rcs)
            ...
            (clim:with-output-as-presentation (stream rc 'map-inspector-script-ref)
              (princ rc stream)))))
      ;; Entrances
      (let ((entrances (game-resource-entrances data)))
        (when entrances
          (terpri stream)
          (clim:with-text-style (stream (clim:make-text-style :sans-serif :bold :normal))
      ...
      ;; Exits
      (let ((exits (game-resource-exits data)))
        (when exits
          (terpri stream)
          (clim:with-text-style (stream (clim:make-text-style :sans-serif :bold :normal))
            (format stream "~&Exits:"))
      ...
      ;; Triggers
      (let ((triggers (game-resource-triggers data)))
        (when triggers
          (terpri stream)
          (clim:with-text-style (stream (clim:make-text-style :sans-serif :bold :normal))
...
(define-map-inspector-frame-command (com-map-inspector-save :menu t :name t) ()
  (let* ((frame clim:*application-frame*)
    ...
    (let* ((old-name (pathname-name old-tmx-path))
           (old-stem (game-resource-display-name data))
      ...
      (clim-sys:make-process
       (lambda ()
         (let ((old-stem (game-resource-display-name data))
               (references '()))
           (dolist (f (directory
  ...
  (define-map-inspector-frame-command (com-map-inspector-copy-json :menu t :name t) ()
    (let* ((frame clim:*application-frame*)
      ...
      (let* ((json-data `(("name" . ,(game-resource-display-name data))
                          ("width" . ,(game-resource-width data))
                          ("height" . ,(game-resource-height data))
                          ("tilesets" .
                                      ,(loop for (gid . src) in (game-resource-tilesets data)
                                             collect (list (cons "firstgid" gid)
                                                           (cons "source" src))))
                          ("spawns" .
                                    ,(loop for (name kind x y) in (game-resource-spawns data)
                                           collect (list (cons "name" name)
                                                         (cons "kind" (string-downcase kind))
                                                         (cons "x" (floor x))
                                                         (cons "y" (floor y)))))
                          ("runCommands" . ,(game-resource-run-commands data))
                          ("entrances" .
                                       ,(loop for (name x y) in (game-resource-entrances data)
                                              collect (list (cons "name" name)
                                                            (cons "x" (floor x))
                                                            (cons "y" (floor y)))))
                          ("exits" .
                                   ,(loop for (target x y) in (game-resource-exits data)
                                          collect (list (cons "target" target)
                                                        (cons "x" (floor x))
                                                        (cons "y" (floor y)))))
                          ("triggers" .
                                      ,(loop for (script x y) in (game-resource-triggers data)
        ...
        (clim-simple-echo:run-in-simple-echo
         ...
         :window-title (format nil "JSON — ~a" (game-resource-display-name data))
         :width 600 :height 400))))
      ...
      (let ((text (with-output-to-string (s)
                    (let* ((display-name (game-resource-display-name data))
                      ...
                      (format s "Dimensions: ~d × ~d~%"
                              (game-resource-width data)
                              (game-resource-height data))
                      (format s "~%Tilesets:~%")
                      (dolist (ts (game-resource-tilesets data))
        ...
        (clim-simple-echo:run-in-simple-echo
         (lambda () (princ text))
         :window-title (format nil "Text — ~a" (game-resource-display-name data))
         :width 600 :height 400)))))
  ...
  (let* ((frame clim:*application-frame*)
    ...
    (let ((text (with-output-to-string (s)
          ...
          (path (prompt-save-pathname
                 (format nil "~a.txt" (game-resource-display-name data))
                 :type "txt")))
      (when path
...
(define-map-inspector-frame-command (com-map-inspector-save-json :menu t :name t) ()
  (let* ((frame clim:*application-frame*)
    ...
    (let* ((plist (game-resource-properties data))
           (prop-alist (loop for (k v) on plist by #'cddr
                              collect (cons (string-downcase (symbol-name k)) v)))
           (json-data `(("name" . ,(game-resource-display-name data))
                         ("suffix" . ,(or (map-inspector-new-suffix frame)
                                          (game-resource-numeric-suffix data)))
                         ("width" . ,(game-resource-width data))
                         ("height" . ,(game-resource-height data))
           ...
           (json-str (json:encode-json-to-string json-data))
           (path (prompt-save-pathname
                  (format nil "~a.json" (game-resource-display-name data))
                  :type "json")))
      (when path
        (with-open-file (f path :direction :output :if-exists :supersede
                            :external-format :utf-8)
                       ...
                       (when triggers
                         ...
                         (dolist (trig triggers)
                           (destructuring-bind (script x y) trig
                             (format s "  ~a @ (~d, ~d)~%" script (floor x) (floor y)))))))))
           (path (prompt-save-pathname
                  (format nil "~a.pdf" (game-resource-display-name data))
                  :type "pdf")))
      (when path
        (let* ((base (pathname-name path))
               ...
               (title (format nil "Map Report: ~a" (game-resource-display-name data)))
          ...
          (with-open-file (ps ps-path :direction :output :if-exists :supersede
            ...
            (dolist (sp (game-resource-spawns data))
              (destructuring-bind (name kind x y) sp
                (declare (ignore kind))
            ...
            (dolist (ent (game-resource-entrances data))
              (destructuring-bind (name x y) ent
                (format ps "~d ~d moveto (E:~a) show~%"
                        (+ 56 (floor (* x 16))) (- 680 (floor (* y 16)))
                        (escape-ps-string name))))
            (format ps "0.5 0 0.5 setrgbcolor~%")
            (dolist (exit (game-resource-exits data))
              (destructuring-bind (target x y) exit
