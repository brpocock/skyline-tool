...
(defmethod score->song (score (format (eql :tia)) frame-rate)
...
(defmethod write-song-binary (hokey-notes (format (eql :tia)) output)
  (let ((fps (frame-rate->fps *batari-frame-rate*))
        ...
        (song-name (let* ((on (pathname-name output))
                     ...
                     (if d1
                         ...
                         on))))
    ;; Sound effects are monophonic (single voice), songs can be polyphonic
    ;; Calculate polyphony after label-prefix is available
    (let ((polyphony (if (string-equal label-prefix "Sound")
                         1  ; Sound effects: single voice only
                         *batari-polyphony*)))  ; Songs: use configured polyphony
      (with-open-file (out output :if-exists :supersede :direction :output)
        (format out ";;; Generated by SkylineTool")
        (format out "~%;;;; Format: Interleaved 4-byte streams (AUDCV, AUDF, Duration, Delay)")
        (format out "~%;;; AUDCV = (AUDC << 4) | AUDV (upper/lower nybbles)")
        (format out "~%;;;; Polyphony: ~d voice~:p~2%" polyphony)
        ...
        (let ((note-count 0)
          ...
          (if (= polyphony 1)
              (format out "~&~a_~a:" label-prefix song-name)
              (format out "~&~a_~a_Voice0" label-prefix song-name))
          (format out " .block")
          (let ((voice0-time 0)
            ...
            (dolist (note voice0-notes)
              (let* ((start-time (or (hokey-note-start-time note) 0))
                ...
                (format out "~%  .byte $~2,'0X, ~2d, ~3d, ~2d  ; AUDC=~d AUDV=~d, AUDF=~d, Duration=~d, Delay=~d"
                        audcv audf duration-frames delay-frames
                        audc volume audf duration-frames delay-frames)
            ...
            (format out "~%  .byte 0,  0,  0,  0  ; End marker (lag=0)")
          ...
          (when (= polyphony 2)
            (format out "~%~a_~a_Voice1: .block" label-prefix song-name)
            (let ((voice1-time 0)
                  (voice1-events (list))
                  (orchestra (get-orchestration)))
                      ...
                      do (let* ((next-event-time (if (< event-index (length voice1-events))
                           ...
                           (if (and (< event-index (length voice1-events))
                               ...
                               (let ((event (elt voice1-events event-index)))
                                 (format out "~%  .byte $~2,'0X, ~2d, ~3d,  0  ; AUDCV=$~2,'0X, AUDF=~d, Duration=~d, Delay=0"
                               ...
                               (when (> gap 0)
                                 (loop while (> gap 0)
                                       do (let ((rest-duration (min gap 255)))
                                            (format out "~% .byte $00,  0, ~3d,  0  ; Rest (AUDC=0 AUDV=0), Duration=~d"
