...
;; For batariBASIC generation, we reuse the Hokey note model and render as .bas data
...
;; Emit a batariBASIC .bas file with data statements representing the Hokey bytes
(defmethod write-song-binary (hokey-notes (format (eql :batariBASIC)) output)
  (declare (ignore hokey-notes))
  (with-open-file (out output :if-exists :supersede :direction :output)
    ...
    (format out "data SongBytes~%")
    (format out "  0, 0, 0, 0, 0, 0, 0, 0~%")
    (format out "end~%"))
  t)
