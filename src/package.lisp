...
(defparameter *game-title* (or (cdr (assoc :*game *project.json*)) "Phantasia"))
(defparameter *part-number* (or (cdr (assoc :*part-number *project.json*)) "TEST"))
(defparameter *studio* (or (cdr (assoc :*studio *project.json*)) "Interworldly Adventuring, LLC"))
(defparameter *publisher* (or (cdr (assoc :*publisher *project.json*)) "Test Publisher"))
(defparameter *machine* (or (cdr (assoc :*machine *project.json*)) 7800))
(defparameter *sound* (or (cdr (assoc :*sound *project.json*)) "HOKEY"))
(defparameter *common-palette* (or (mapcar #'intern (cdr (assoc :*common-palette *project.json*)))
                                  '(PEACH PURPLE GREEN SILVER ORANGE BROWN WHITE GRAY BLACK YELLOW RED BLUE)))
(defparameter *default-skin-color* (or (cdr (assoc :*default-skin-color *project.json*)) "peach"))
(defparameter *default-hair-color* (or (cdr (assoc :*default-hair-color *project.json*)) "black"))
(defparameter *default-clothes-color* (or (cdr (assoc :*default-clothes-color *project.json*)) "blue"))
