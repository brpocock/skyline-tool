...
(defun render-maria-to-rgb (dump mode address width colors)
  ...
  (let* ((ppb (ecase mode (:160a 4) (:160b 2)))
         ...
         (rgb (make-array (* tw th 3) :element-type '(unsigned-byte 8) :initial-element 0)))
    (dotimes (y th)
      (dotimes (x tw)
        (let* ((pen (aref fp x y))
               (reg (elt colors pen))
               (col (if (and (integerp reg) (<= 0 reg 255) (nth reg system-palette))
                        ...
                        '(0 0 0))))
          (setf (aref rgb (* (+ (* y tw) x) 3)) (first col)
                (aref rgb (+ (* (+ (* y tw) x) 3) 1)) (second col)
                (aref rgb (+ (* (+ (* y tw) x) 3) 2)) (third col)))))
    (values tw th rgb)))
