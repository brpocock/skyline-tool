...
(defun read-map-labels (&optional (pathname #p"./Object/7800/Bank00.Public.NTSC.o.LABELS.txt"))
...
(defun decode-map (&optional (dump (load-dump-into-mem)))
  (let ((addresses (read-map-labels)))
    (labels
      ...
      (format t "~&Tileset bank: $~2,'0x = ~a" (dump-peek "TilesetBank" dump)
              (case (dump-peek "TilesetBank" dump)
                (5 "Sandy Island")
                (6 "Indoor")
                (7 "Jungle Island")
                (8 "Ancient")
