               ...
               (uiop/lisp-build:compile-file-error
                 (lambda (c)
                   (format *error-output*
                           ...
                           c)
                    (finish-output *error-output*)))))
  ;; Attempt to load skyline-tool (retry once if stale fasl cache causes issues)
  (or (handler-case (ql:quickload :skyline-tool :silent t)
        (unbound-variable (e)
          (format *error-output* "~&Non-fatal unbound variable (~a), retrying…~%" e)
          (finish-output *error-output*)
          nil)
        (uiop/lisp-build:compile-file-error (e)
          (format *error-output* "~&Note: ~a~%" e)
          (finish-output *error-output*)
          nil)
        (error (e)
          (format *error-output* "~&Load issue (~a), retrying…~%" e)
          (finish-output *error-output*)
          nil))
      (handler-case (ql:quickload :skyline-tool :silent t)
        (unbound-variable (e)
          (format *error-output* "~&Note: ~a~%" e)
          (finish-output *error-output*)
          nil)
        (error (e)
          (format *error-output* "~&Failed: ~a~%" e)
          (finish-output *error-output*)
          nil)))
  (unless (find-package :skyline-tool)
    (format *error-output* "~&Warning: skyline-tool not loaded.~%")
    (finish-output *error-output*))
